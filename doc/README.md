...
- `doc/README-no-taie.md`：去 Tai-e 主链化设计（建库主链、迁移阶段与验收指标）
- `doc/README-no-taie-phase0.md`：Phase 0 实施文档与 issue 清单（基线测量、dev 内核迁移准备）
- `doc/README-no-taie-dev-kernel-inventory.md`：`dev` 分支可迁移分析内核白名单（迁/拆迁/不迁裁决）
- `doc/README-no-taie-fact-snapshot.md`：`BuildContext -> BuildFactSnapshot` 适配层设计（输入 facts 与边输出分离）
