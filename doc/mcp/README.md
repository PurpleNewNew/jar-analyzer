...
2. GUI 已启动内置 HTTP API（默认 `10032`，可在 GUI 的 `API Startup Config` 中调整并重启生效）
...
1. **API Token**（保护 `/api/*`）：在 GUI 的 `API Startup Config` 中启用 `auth` 并设置 `token`，API 请求头 `Token: <TOKEN>`
