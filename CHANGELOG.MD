...
## 5.12 - 已发布——我fork一份学习自用，感谢jar-analyzer慷慨开源

更新日志：

- [功能] `DFS` 分析允许导出 @4ra1n
- [功能] `DFS` 分析允许设置最大次数防止分析不停 @4ra1n
- [功能] `DFS` 分析允许设置黑名单类跳过分析 @4ra1n
- [BUG] 修复污点分析黑暗主题 `UI` 问题 @4ra1n
- [优化] 启动 `MCP` 时应该输出更多有意义的信息 @4ra1n
- [BUG] 完整修复 `Sanitizer` 的语义问题 @PurpleNewNew
- [BUG] 修复 `Sanitizer` 的匹配规则并提高了反编译缓存容量 @PurpleNewNew
- [功能] 新增带 `Redis` 的工作流配置 @PurpleNewNew
- [优化] 更新 `n8n` 工作流 @PurpleNewNew
- [功能] `/api/cfr_code` 与 `/api/fernflower_code` 支持 `includeFull/full` 参数控制 `fullClassCode` @PurpleNewNew
- [BUG] 修复 `MCP` 的并发锁问题 @PurpleNewNew
- [BUG] 修复 `methodDesc` 未参与匹配的问题 @PurpleNewNew

感谢以下用户的贡献:

- 4ra1n (https://github.com/4ra1n)
- PurpleNewNew (https://github.com/PurpleNewNew)
...
- `mcp-build.zip` 包含所有系统的 `mcp` 一般使用 `mcp_windows_amd64.exe`
- `report-mcp-build.zip` 配合 `workflow` 使用的自动报告生成 `mcp`
...
`MCP` 文档：https://github.com/jar-analyzer/jar-analyzer/tree/master/mcp-doc

## 5.12 - 已发布最新版

更新日志：

- [功能] `report-mcp` 支持保存数据到 `sqlite` @4ra1n
- [优化] 优化 `report-mcp` 界面和功能 @4ra1n
- [优化] 限制字符串查询返回数量防止 `MCP` 卡死 @4ra1n
- [其他] 所有依赖升级到支持 `Java 8` 的最新版 @4ra1n
...
一些安全改进吗，并提供大模型 `workflow` 审计文档，参考主页 `README`

特别致谢： whwlsfb L-codes osword 三位大佬

更新日志：

- [重要] 分享 `n8n workflow` 配置和文档 @whwlsfb
- [MCP] 优化部分代码提高 `AI` 分析效率 @whwlsfb
- [MCP] 提供结合 `n8n workflow` 的 `report-mcp` @whwlsfb
- [MCP] 支持 `-debug` 选项打印被调用日志 @4ra1n
- [MCP] 允许通过 `-ja -jt` 设置 `API Token` @4ra1n
- [MCP] 允许通过 `-auth -token` 设置 `MCP Token` @4ra1n
- [功能] 新增一个面板展示 `API` 配置信息和文档 @4ra1n
- [功能] 安全模式 `--security` 敏感操作提醒用户确认 @4ra1n
- [BUG] 修复某些罕见情况发生的 `SQL` 报错问题 @4ra1n
- [优化] 允许 `API` 通过 `-sa -st` 设置 `Token` @4ra1n
- [优化] 允许 `API` 指定 `bind` 地址默认 `0.0.0.0` @4ra1n
- [其他] 升级内置 `JRE` 到 `8u472` 和 `21.0.9` @4ra1n
- [其他] 所有依赖升级到支持 `Java 8` 的最新版 @4ra1n

感谢以下用户的贡献:

- 4ra1n (https://github.com/4ra1n)
- whwlsfb (https://github.com/whwlsfb) (重要 AI 贡献)
...
- [重要] 支持 `MCP` @4ra1n
- [功能] `API` 完善补充 @4ra1n @7eleven
- [BUG] `API` 路径处理存在混淆问题 @4ra1n
- [BUG] 修复多次点击按钮导致数据重复问题 @4ra1n
...
- [BUG] 解决重新分析新文件时数据库占用报错 @4ra1n
- [BUG] 污点分析未通过的 `case` 也应该展示 @4ra1n
- [优化] `DFS` 每次重新分析应该清除上一次记录 @4ra1n
- [优化] 补充 `JSON.parseArray` 搜索规则 @4ra1n
- [其他] 所有依赖升级到支持 `Java 8` 的最新版 @4ra1n
...
- [功能] `DFS` 分析 `sink` 支持从文件 `rules/dfs-sink.json` 加载 @4ra1n
