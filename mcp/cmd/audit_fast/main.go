...
const (
	version = "1.2.1"
	name    = "jar-analyzer-mcp-audit-fast"
)
...
func main() {
	fmt.Println("     ____.               _____                .__          " +
		...
		"              \\/              \\/     \\/     \\/     \\/     " +
		"       \\/    \\/       ")
	fmt.Println("jar-analyzer-mcp-audit-fast (https://github.com/jar-analyzer/jar-analyzer)")
	fmt.Printf("version: %s usage: %s\n", version, "[mcp-audit-fast.exe -port 20033 -url http://127.0.0.1:10032]")
	...
	flag.IntVar(&port, "port", 20033, "port to listen on")
	...
	fmt.Println("-----------------------------------------------------------")
	fmt.Println("[INFO] Starting Jar Analyzer MCP Audit-Fast Server...")
	fmt.Println("[信息] 正在启动 Jar Analyzer MCP Audit-Fast 服务器...")
	fmt.Println("-----------------------------------------------------------")
	...
	tools.RegisterAuditFastTools(s)
