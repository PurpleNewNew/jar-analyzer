...
const (
	version = "1.2.1"
	name    = "jar-analyzer-mcp-vul-rules"
)
...
func main() {
	fmt.Println("     ____.               _____                .__          " +
		...
		"              \\/              \\/     \\/     \\/     \\/     " +
		"       \\/    \\/       ")
	fmt.Println("jar-analyzer-mcp-vul-rules (https://github.com/jar-analyzer/jar-analyzer)")
    fmt.Printf("version: %s usage: %s\n", version, "[mcp-vul-rules.exe -port 20037 -url http://127.0.0.1:10032]")
    ...
    flag.IntVar(&port, "port", 20037, "port to listen on")
	...
	fmt.Println("-----------------------------------------------------------")
	fmt.Println("[INFO] Starting Jar Analyzer MCP Vul-Rules Server...")
	fmt.Println("[信息] 正在启动 Jar Analyzer MCP Vul-Rules 服务器...")
	fmt.Println("-----------------------------------------------------------")
	...
	tools.RegisterVulRulesToolsLine(s)
