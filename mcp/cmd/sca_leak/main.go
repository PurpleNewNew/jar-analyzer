...
const (
	version = "1.2.1"
	name    = "jar-analyzer-mcp-sca-leak"
)
...
func main() {
	fmt.Println("     ____.               _____                .__          " +
		...
		"              \\/              \\/     \\/     \\/     \\/     " +
		"       \\/    \\/       ")
	fmt.Println("jar-analyzer-mcp-sca-leak (https://github.com/jar-analyzer/jar-analyzer)")
	fmt.Printf("version: %s usage: %s\n", version, "[mcp-sca-leak.exe -port 20036 -url http://127.0.0.1:10032]")
	...
	flag.IntVar(&port, "port", 20036, "port to listen on")
	...
	fmt.Println("-----------------------------------------------------------")
	fmt.Println("[INFO] Starting Jar Analyzer MCP SCA/Leak Server...")
	fmt.Println("[信息] 正在启动 Jar Analyzer MCP SCA/Leak 服务器...")
	fmt.Println("-----------------------------------------------------------")
	...
	tools.RegisterScaLeakToolsLine(s)
