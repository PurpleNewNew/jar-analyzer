...
4. `ja.taint.track` 现支持显式 `direction=forward|backward|bidirectional`；旧的 `mode=source|sink` 继续兼容并映射到 `forward/backward`。当 `direction=backward` 且 `searchAllSources=true` 时会直接复用最新 source 规则做逆向剪枝搜索
