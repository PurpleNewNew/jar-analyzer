...
11. 非 all-common 场景下，选中的调用图引擎失败会直接终止建库；项目保持 `rebuild required`，不回退旧调用图/旧快照

规则热刷新补充：
1. `rules/source.json` / `rules/model.json` / `rules/sink.json` 变更后均可热生效
2. DFS 的 `searchAllSources` / `onlyFromWeb` 会按“当前 source 真相”选 source：复用最新 `source/sourceAnnotations` 规则，并叠加当前项目稳定的显式 Web/RPC 入口事实；不会再对旧的持久化 `source_flags` 做兜底回退，不要求先重建项目
3. 图中的持久化 `source_flags` 仍属于建库产物，会在下一次成功建库后刷新，但 DFS/Taint 的默认 source 选择已不再把它当成 current-truth
4. `ja.taint.track` 现支持显式 `direction=forward|backward|bidirectional`；旧的 `mode=source|sink` 继续兼容并映射到 `forward/backward`。当 `direction=backward` 且 `searchAllSources=true` 时会直接按当前 source 真相做逆向剪枝搜索
5. `rules/model.json` 新增 `pruningPolicy`：用于调整 `sourceSelection`、`sanitizerMode`、`allowAdditionalFlows`、`confidenceMode`；默认 `sourceSelection=rules`，即按当前 source 真相剪枝。若确实需要叠加历史建库标记，需显式设为 `merged`；支持 `scenarios[]` 按 `sinkKind/sinkTier/sinkTags + mode + searchAllSources` 做分场景覆盖，后写覆盖前写
...
3. 若输入全部命中 common library，固定 `continue-no-callgraph`
4. 非 all-common 场景不再回退 bytecode 调用图
5. 项目库写入始终走 staging + atomic swap；失败不会先删旧库
6. `precision` 会把 semantic/reflection/trigger/high-fanout 调用点纳入选择性高精度 PTA，并在 `callgraph` stage metrics 中暴露 `pta_precision_*` 计数
