...
7. 规则体系统一：`rules/sink.json`（sink 检索 + sink kind 分类）、`rules/source.json`（source/sourceAnnotations）、`rules/model.json`（summary/sanitizer/guard/additional）
8. 项目模型已切换为 `TEMP/PERSISTENT`（不再有 `default` 业务项目）
9. 首次进入新模型会清理旧项目库与旧注册表，不保留历史数据
10. 非 all-common 场景下，Tai-e 失败会直接终止建库（不再回退 bytecode 调用图）
...
4. `.jar-analyzer-projects-v2-reset.done`：新项目模型一次性重置标记
5. `jar-analyzer-temp/`：临时目录（解包/缓存）
6. `.jar-analyzer`：本地配置文件（properties，包含 API/MCP 启动配置等）
7. `logs/`：日志（如启用）
