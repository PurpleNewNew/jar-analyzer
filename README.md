...
5. `jar.analyzer.taint.propagation` 仅支持 `strict|balanced`，并按当前系统属性即时解析，不要求重启后才生效
...
2. `jar.analyzer.taint.propagation` 会按当前系统属性即时解析；summary 指纹也会绑定当前传播模式，不再把类加载时的静态缓存误当成运行态配置
...
3. 若配置了非法 `jar.analyzer.callgraph.profile`，会直接拒绝启动当前建库
4. 若输入全部命中 common library，固定 `continue-no-callgraph`
5. 非 all-common 场景固定要求字节码调用图主链成功
