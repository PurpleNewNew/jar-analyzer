...
/**
 * Expression that executes a pipe on evaluation. Returns the value of [[columnNameToGet]] column of the first row produced by the pipe.
 */
case class NestedPipeGetByNameExpression(
  pipe: Pipe,
  ...
  override def apply(row: ReadableRow, state: QueryState): AnyValue = {
    ...
    results.close()
    resultRow.getByName(columnNameToGet)
  }
