 ...
 */
package org.neo4j.cypher.internal.runtime.core.commands.predicates
...
import org.neo4j.cypher.internal.runtime.core.commands.expressions.AbstractCachedNodeHasProperty
import org.neo4j.cypher.internal.runtime.core.commands.expressions.AbstractCachedNodeProperty
import org.neo4j.cypher.internal.runtime.core.commands.expressions.AbstractCachedProperty
...
import org.neo4j.cypher.internal.runtime.core.commands.values.KeyToken
...
case class HasDynamicLabels(entity: Expression, labels: Seq[Expression]) extends Predicate {
  ...
  def isMatch(ctx: ReadableRow, state: QueryState): IsMatchResult = {
    entity(ctx, state) match {
      ...
      case value =>
        IsMatchResult(CypherFunctions.hasDynamicLabels(
          value,
  ...
  def rewrite(f: Expression => Expression): Expression =
    f(HasDynamicLabels(entity.rewrite(f), labels.map(_.rewrite(f))))

  def arguments: collection.Seq[Expression] = entity +: labels
