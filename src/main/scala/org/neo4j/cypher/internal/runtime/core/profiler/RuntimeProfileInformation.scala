 ...
 */
package org.neo4j.cypher.internal.runtime.core.profiler
...
class RuntimeProfileInformation extends QueryProfile {
  ...
  def snapshot: RuntimeProfileInformationSnapshot = {
    ...
    RuntimeProfileInformationSnapshot(currentDbHitsMap, currentRowsMap)
  }

  def aggregatedSnapshot: RuntimeProfileInformationAggregatedSnapshot = {
    val aggregatedDbHits = dbHitsMap.values.map(_.count).sum
    RuntimeProfileInformationAggregatedSnapshot(aggregatedDbHits)
  }
  ...
  def merge(other: RuntimeProfileInformation): Unit = {
...
case class RuntimeProfileInformationSnapshot(dbHitsMap: collection.Map[Id, Long], rowsMap: collection.Map[Id, Long])

case class RuntimeProfileInformationAggregatedSnapshot(dbHits: Long)
