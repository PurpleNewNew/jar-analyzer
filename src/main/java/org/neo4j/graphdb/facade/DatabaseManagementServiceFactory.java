...
public class DatabaseManagementServiceFactory {
    ...
    public DatabaseManagementService build(Config config, boolean daemonMode, final ExternalDependencies dependencies) {
        ...
        // neo4lite: embedded-only runtime, network services are intentionally disabled at source level.
        globalDependencies.satisfyDependency(new EmbeddedOnlyNetworkSurface());
    ...
    private static void dumpDbmsInfo(InternalLog log, GraphDatabaseAPI system) {
        try {
            log.info("dbms.runtime: neo4lite-embedded");
            log.info("dbms.version: " + Version.getNeo4jVersion());
            log.info("db.name: " + system.databaseName());
        } catch (Exception e) {
            log.info("Unable to dump DBMS information", e);
        }
    }

    // Marker dependency used for diagnostics in embedded-lite builds.
    private static final class EmbeddedOnlyNetworkSurface {}
}
