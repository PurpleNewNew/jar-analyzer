...
public final class SwingResultHtml {
    ...
    public static String renderClassRow(String className, List<String> tokens) {
        String normalizedClass = normalizeClassName(className).trim();
        if (normalizedClass.isEmpty()) {
            return wrapHtml("-");
        }
        String body = span(highlightText(normalizedClass, tokens), "#d48806", true);
    ...
    public static String renderMethodRow(String className,
        ...
        if (!normalizedClass.isEmpty()) {
            body.append(span(highlightText(normalizedClass, tokens), "#d48806", true))
                    .append("<span style=\"color:#7a7a7a;\">#</span>");
        }
        body.append(highlightText(methodSig, tokens));
        if (lineNumber > 0) {
            body.append("<span style=\"color:#6f6f6f;\"> : ")
                    .append(lineNumber)
                    .append("</span>");
        }
        ...
        if (values != null) {
            for (String value : values) {
                ...
                for (String part : parts) {
                    ...
                    if (token.isEmpty() || token.length() > 96) {
                        continue;
                    }
                    out.add(token);
                }
