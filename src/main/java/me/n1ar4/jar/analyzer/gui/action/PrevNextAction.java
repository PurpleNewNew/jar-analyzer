...
public class PrevNextAction {
    public static void run() {
        ...
        instance.getPrevBtn().addActionListener(e -> {
            ...
            if (prev.getMethodName() == null || prev.getMethodName().isEmpty()
                    || prev.getMethodDesc() == null || prev.getMethodDesc().isEmpty()) {
                NavigationHelper.OpenClassOptions classOptions = NavigationHelper.OpenClassOptions.defaults()
                        .openInNewTab(false)
                        .refreshContext(true)
                        .recordState(false);
                NavigationHelper.openClass(prev.getClassName(), prev.getJarName(), classOptions);
                return;
            }
        ...
        instance.getNextBtn().addActionListener(e -> {
            ...
            if (next.getMethodName() == null || next.getMethodName().isEmpty()
                    || next.getMethodDesc() == null || next.getMethodDesc().isEmpty()) {
                NavigationHelper.OpenClassOptions classOptions = NavigationHelper.OpenClassOptions.defaults()
                        .openInNewTab(false)
                        .refreshContext(true)
                        .recordState(false);
                NavigationHelper.openClass(next.getClassName(), next.getJarName(), classOptions);
                return;
            }
            MethodResult m = new MethodResult();
