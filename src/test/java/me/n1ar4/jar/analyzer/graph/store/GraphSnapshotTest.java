...
class GraphSnapshotTest {
    @Test
    void findMethodNodeIdShouldRejectAmbiguousLooseMatch() {
        ...
        GraphNode node4 = new GraphNode(4L, "method", 3, "a/C", "bar", "()V", "", -1, -1);
        GraphSnapshot snapshot = GraphSnapshot.of(
                7L,
                Map.of(1L, node1, 2L, node2, 3L, node3, 4L, node4),
                Map.of(),
                Map.of(),
                Map.of()
        );
        ...
        assertEquals(4L, snapshot.findMethodNodeId("a/C", "bar", "()V", null));
        assertEquals(-1L, snapshot.findMethodNodeId("a/B", "bar", "()V", 1));
        assertEquals(Set.of(1L, 2L, 4L), snapshot.getNodesByKindView("method")
